...
This document carried no `Version History` before this change, so no earlier document revision is recoverable from the repository; the row above is the first one recorded rather than a claim that the document is new.
...
- **Generated-site version string** \- `docs/generate_docs.sh` derives `PROJECT_VERSION` from `git describe --tags`, which at the revision this document describes yields `1.3.0-3-g1582f3b`. That form is a tag, the number of commits since it, and an abbreviated hash \- a build identifier, not a released version.
...
- **The bridge record is caller-allocated.** `bridgeDetails` is passed by pointer to `updateBridgeInfo`, `HandlePreConfigVendor` and `HandlePostConfigVendor`. The caller allocates it, populates it, and owns it for the duration of the call. No function in this interface creates or destroys one.
...
- Modules must not retain a pointer to a caller-supplied `bridgeDetails` record beyond the call it was passed to, because the caller owns that storage and this interface gives the implementation no means of learning when it is released.
...
**The interface also supplies the logging mechanism.** `bridge_util_log` is a variadic macro that formats a message and writes it through the caller-supplied `logFp` stream, falling back to standard output when that stream is not open. It timestamps every entry in `UTC` and emits the fixed form `YYYY-MM-DD HH:MM:SS ::: <message>`. Two declared constants bound it: `MAX_LOG_BUFF_SIZE` limits the formatted message and `TIMESTAMP` reserves room for the prefix. The header records that this logging mechanism is intended to be made generic in a future revision of the interface, so a caller should not build on its internals beyond the stream and the format above.
...
Each `API` interface is versioned using [Semantic Versioning 2.0.0](https://semver.org/), and the vendor code complies with a specific version of the interface.
...
**The `OVS` dependency is being removed, and the removal is still in progress.** This interface was historically coupled to Open vSwitch, and upstream **has been removing** that dependency \- the progressive form is deliberate and accurate. The repository's own history shows why: the `1.1.1` release removed the `OVS` agent `API` header from `bridge_util_hal.h`, and the `1.2.0` release carries the dependency-removal work including one commit that was reverted and then re-applied. The surviving evidence in the interface itself is the type name `Gateway_Config_Non_Ovs_Bridge`, which names the non-`OVS` arrangement explicitly, and the stated purpose of `network_interface.h`, which exists to eliminate the build-time dependency on bridge utilities from `OVS`.

One residue of that history is visible in the interface documentation and a caller should know to disregard it: the parameter documentation on `updateBridgeInfo` refers to an operation enumeration that **this interface does not declare**. The range a caller must actually use for that argument is `enum BridgeOpr`. See `State-Dependent Behavior`.
